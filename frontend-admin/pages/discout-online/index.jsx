...
const DiscoutOnline = () => {
    ...
    const orders = useMemo(() => {
        return [...orderList.filter(order => ['QR'].includes(order?.type)).slice(pagesVisited, pagesVisited + usersPerPage)]
    }, [orderList, pagesVisited, usersPerPage])

    const pageCount = Math.ceil(orderList.filter(order => ['QR'].includes(order?.type)).length / usersPerPage);
