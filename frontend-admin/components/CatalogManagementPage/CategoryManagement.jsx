    ...
    const handleDelete = async (id) => {
        swalert
            .fire({
                ...
                icon: "warning",
                text: "Bạn muốn danh mục sản phẩm này?",
                showCloseButton: true,
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    try {
                        const {data} = await axios.delete('http://localhost:8080/api/category/delete',
                            { data: { category_id: id } })
                            if(data?.success) {
                                refreshCategoryTable()
                                swtoast.success({
                        ...
                        refreshCategoryTable()
                        swtoast.error({
                            text: 'Xảy ra lỗi khi Xóa danh mục sản phẩm vui lòng thử lại!'
                        })
                    }
                }
            })
                            ...
                            categoryList.map((category, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td onClick={async () => await handleDelete(category?.category_id)} style={{color: 'red', cursor: 'pointer'}}>Xóa</td>
                                    </tr>
                                )
                            })
                        }
