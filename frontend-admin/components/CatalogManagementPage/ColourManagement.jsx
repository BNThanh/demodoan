        ...
        swalert
            ...
            .then(async (result) => {
                if (result.isConfirmed) {
                    try {
                        const {data} = await axios.delete('http://localhost:8080/api/colour/delete',
                            { data: { colour_id: id } })
                            if(data?.success) {
                                refreshColourTable()
                                swtoast.success({
                            ...
                            colourList.map((colour, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td onClick={async () => await handleDelete(colour?.colour_id)} style={{color: 'red', cursor: 'pointer'}}>Xóa</td>
                                    </tr>
                                )
                            })
                        }
