...
const ProductDetailPage = () => {
	...
	useEffect(() => {
		const handleGetProductVariant = async () => {
			try {
				...
				const xxx = await axios.get(backendAPI + `/api/feedback/list/${respond.data.product_variant_id}`);
				setFeedbackList(xxx.data)
	...
	}, [selectedColorIndex, handleGetProduct, selectedSizeIndex]);
...
return x + y;
}, 0) / feedbackList?.length) || 0} / 5 <span className='star-icon'><StarFilled /></span></h5>
				</div>
				<div>
