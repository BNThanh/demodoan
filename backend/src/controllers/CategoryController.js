...
let deleteCategory = async (req, res, next) => {
    ...
    try {
        const des = await Category.destroy({ where: { category_id: category_id } })
        if(des) {
            return res.send({ message: 'Xóa caregory_id thành công', success: true })
        }
        return res.send({ message: 'Xóa caregory_id thành sai', success: false })
    } catch (err) {
        console.log(err)
        return res.status(500).send('Gặp lỗi khi tải dữ liệu vui lòng thử lại');
    }
