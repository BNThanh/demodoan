...
let createFeedBack = async (req, res, next) => {
    ...
    try {
        var productVariant = await Product_Variant.findOne({ where: { product_variant_id: product_variant_id } });
        if (productVariant == null) return res.status(400).send('Product Variant này không tồn tại');
    } catch (err) {
        console.log(err);
        return res.status(500).send('Gặp lỗi khi tải dữ liệu vui lòng thử lại');
    }
    console.log({ user_id: customer_id, product_variant_id, rate, content })
    try {
